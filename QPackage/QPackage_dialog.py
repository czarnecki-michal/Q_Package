                    ...
                    if(layer.name() == row.text()):
                        ...
                        if layer.type() == QgsMapLayer.VectorLayer:
                            ...
                            # write the qgis layer to the destination directory
                                qgis_layer_source = layer.source().split("|")[0]
                                qgis_layer_extension = qgis_layer_source[-4:].lower()
                                qgis_layer_name = os.path.basename(qgis_layer_source)
                                if os.name == 'nt':
                                    if qgis_layer_extension == ".tab":
                                        QgsVectorFileWriter.writeAsVectorFormat(layer, self._repertoire.text() + "\\" + qgis_layer_name, "utf-8", QgsCoordinateReferenceSystem(projection), "Mapinfo File")
                                    else:
                                        QgsVectorFileWriter.writeAsVectorFormat(layer, self._repertoire.text() + "\\" + qgis_layer_name, "utf-8", QgsCoordinateReferenceSystem(projection), "ESRI Shapefile")
                                else:
                                ...
                                if layer.type() == QgsMapLayer.VectorLayer:
                                    ...
                                    for coucheprojet in maplayers:
                                        ...
                                        if coucheprojetnom == layer.name():
                                            ...
                                            replaceText(coucheprojet.getElementsByTagName('datasource')[0], "./"+qgis_layer_name)
                                            replaceText(coucheprojet.getElementsByTagName('provider')[0], "ogr")
                                            coucheprojet.getElementsByTagName('provider')[0].attributes["encoding"].value = "UTF-8"
                                            pr = "<spatialrefsys>"
